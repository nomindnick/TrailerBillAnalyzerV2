...
class ReportGenerator:
    ...
    def generate_report(
        ...
        try:
            ...
            self.logger.error(f"Error generating {output_format} report: {str(e)}")
            raise
    ...
    def _generate_header(self, bill_info: Dict[str, Any]) -> str:
        """
        Generate the report header with safe date handling.
        """
        try:
            date_approved = (
                bill_info['date_approved'].strftime('%Y-%m-%d') 
                if bill_info.get('date_approved') 
                else 'Not Available'
            )
    ...
    # Add other existing helper methods here (e.g., _format_analysis_section_html, 
    # _generate_practice_group_relevance, etc.)
    ...
    def _extract_lines(self, raw_text: str) -> List[str]:
        """Extract non-empty lines from text, stripped of XML tags."""
        if not raw_text:
            return []
    ...
    def _process_subheadings_and_bullets_html(self, lines: List[str]) -> str:
        """Process lines into HTML with subheadings and bullets."""
        html_output = []
        in_list = False
    ...
    def _generate_practice_group_relevance(self, change: Dict[str, Any], practice_group: str) -> str:
        """Generate explanation of practice group relevance."""
        pg_entry = next(
        ...
        relevance_explanations = {
            ...
            # Add other practice group entries as needed
        }
    ...
    def _generate_no_impact_summary(self, change: Dict[str, Any]) -> str:
        ...
        is_state_only = any(indicator.lower() in digest_text.lower() 
                          for indicator in state_only_indicators)
    ...
    def _generate_practice_area_section_text(self, area: str, changes: List[Dict[str, Any]]) -> str:
        ...
        for change in changes:
            # Skip changes not relevant to this practice area
    ...
    def _process_subheadings_and_bullets_text(self, lines: List[str]) -> str:
        """Process lines for text output with subheadings and bullets."""
        output_lines = []
